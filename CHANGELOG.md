...
### Added
- **Import GoPlayAlong projects (`.gp` + audio + a GoPlayAlong `.xml`).** A GoPlayAlong export is a `<track>` **sync sidecar** — it points at a Guitar Pro score and an audio file and stores the bar→audio sync points, but carries **no chart** — so feeding it to the arrangement importer failed with "not a recognised EOF arrangement XML". New `goplayalong.py` parser + `/api/plugins/editor/parse-goplayalong-sync` endpoint turn that XML into the same `sync_points` / `audio_offset` shape `autosync-gp` / `extract-gp-sync` already return, so the existing `convert-gp` path applies GoPlayAlong's **authored** sync to the referenced `.gp` instead of re-deriving it via onset detection. This slice is the parser + endpoint (Tests: `tests/test_goplayalong.py`, verified against a real GoPlayAlong export — count header, per-bar audio-time/tempo mapping, bar-1 offset, malformed handling); the New-dialog UI hook that routes a dropped GoPlayAlong `.xml` through this path lands next.
